...
public class Point {
	...
	public void project2(V3 entpos, V3 campos, V3 camori, V3 axisori){ //same as project3 but with a second rotation
		//supposed to rotate visual angle with entity rotation
		...
		V3 v = new V3(Math_methods.rotatepoint(Math_methods.reverserotatepoint(position.add2(entpos).sub2(campos), axisori), camori));
	...
	public void projectabs2(V3 campos, V3 camori, V3 axisori){ //same as projectabs but with a second rotation
		//used to render equipotentials, which are computed in barycentric coordinates
		//computes 2d visual field position of a point from its 3d position
		...
		V3 v = new V3(Math_methods.rotatepoint(Math_methods.reverserotatepoint(position.sub2(campos), axisori), camori)); //slightly faster since it doesn't have to shift reference frames 
